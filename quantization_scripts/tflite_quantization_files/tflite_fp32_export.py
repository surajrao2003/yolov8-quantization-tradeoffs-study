"""
YOLOv8 .pt → FP32 TFLite.

Run:

  python quantization_scripts/tflite_quantization_files/tflite_fp32_export.py --weights models/yolo_models/yolov8n.pt --imgsz 640

No args defaults to repo yolov8n + 640.

Writes models/tflite_quantized_models/FP32_tflite_models/<imgsz>/
"""
...
def export_fp32_tflite(*, weights: Path, imgsz: int, repo: Path = REPO) -> Path:
    weights = resolve_weights(weights)
    out_dir = tflite_out_dir(repo, "FP32_tflite_models", imgsz)
    dest = out_dir / f"{weights.stem}_{imgsz}_fp32.tflite"
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Export YOLOv8 .pt to FP32 TFLite (Ultralytics).")
    ...
    path = export_fp32_tflite(weights=weights, imgsz=imgsz, repo=REPO)
